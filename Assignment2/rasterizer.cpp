...
{   
    ...
    Vector3f PA_cross_AB = (A - P).cross(AB);
    Vector3f PB_cross_BC = (B - P).cross(BC);
    Vector3f PC_cross_CA = (C - P).cross(CA);

    return PA_cross_AB.dot(PB_cross_BC) > 0 && PA_cross_AB.dot(PC_cross_CA) > 0;
